...
export default function ProdutosubprodutoManutencao({ acao }) {
    ...
    useEffect(() => {
        if(acao.tipo != 'edit') {
            fetch(`http://127.0.0.1:8000/api/produto/manutencao?acao=show&chave=${acao.chave}`, {method: 'GET'})
            .then(res => res.json())
            .then(data => {
                setForm({
                    ...form,
                    'produto_pai_id' : data.registro.id,
                    'produto_pai' : data.registro.nome
                })
            })
            setLoading(false)
            return
        ...
        .then(data => {
            setForm({
                id: data.registro.id,
                produto_pai_id : data.registro.produto_pai_id,
                produto_pai : data.registro.produto_pai.nome,
                produto_filho_id: data.registro.produto_filho_id,
                produto_filho: data.registro.produto_filho.nome,
                ...
                <>
                    ...
                    <form id={`form-${TELA_ID}`}>
                        ...
                        <div className="px-5 mt-2 text-black">
                            <Externo readonly handleChange={handleChange} values={{codigo: form['produto_pai_id'], nome: form['produto_pai']}} title="Produto Pai" campoCodigo="produto_pai_id" campoNome="produto_pai" paginaExterno="produtoConsulta" />
                        </div>
                        <div className="px-5 mt-2 text-black">
                            <Externo handleChange={handleChange} values={{codigo: form['produto_filho_id'], nome: form['produto_filho']}} title="Subproduto" campoCodigo="produto_filho_id" campoNome="produto_filho" paginaExterno="produtoConsulta" />
                        </div>
